...
public class OperationLogService {
	...
	public boolean updateOperationLog(int operationId,Fadmin auditor) throws RuntimeException{
		...
		wallet.setFtotalRmb(MathUtils.add(wallet.getFtotalRmb(), amount));
		...
		int userId = operationLog.getFuser().getFid();
		Fuser fuser = this.userDAO.findById(userId);
		if(fuser.getFgrade() == null){
			fuser.setFgrade(UserGradeEnum.LEVEL1);
