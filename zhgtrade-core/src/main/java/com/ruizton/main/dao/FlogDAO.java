...
public class FlogDAO extends HibernateDaoSupport {
	 ...
	 * @return
	 */
	public String lastDeviceBeforeCode(int userId){
		log.debug("find user last device");
		try {
			String sql = "select fkey5 from flog where fkey1 = ? order by fid desc limit 1";
			Query query = getSession().createSQLQuery(sql);
			query.setParameter(0, userId);
			return (String) query.uniqueResult();
		}catch (RuntimeException re){
			log.error("find user last device", re);
			throw re;
		}
	}
}