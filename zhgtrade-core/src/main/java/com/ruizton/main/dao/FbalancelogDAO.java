...
public class FbalancelogDAO extends HibernateDaoSupport {
	...
	public Map<String,Double> getReport() {
		...
		log.debug("finding Fbalancelog instance with filter");
		try {
			String queryString = "SELECT SUM(a.famount) amt,a.ftype,b.fname from fbalancelog a left outer join fvirtualcointype b " +
					"on a.fvid=b.fid GROUP BY b.fname,a.ftype";
			...
			if(list != null && list.size() >0){
				for(int i=0;i<list.size();i++){
					if(list.get(i) != null){
						...
						map.put(o[2].toString()+"-"+o[1].toString(),Double.valueOf(o[0].toString()));
					}
				}
			}
		} catch (RuntimeException re) {
