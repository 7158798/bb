...
//	（累计充值金额-累计提现金额-账户可用金额-账户可用冻结金额）/(账户可用金豆+账户冻结金豆)
	...
	public String getIntrolString(int fuserid) {
		...
		List list = null;
		StringBuffer sf = new StringBuffer();
		sf.append("SELECT fgrade,COUNT_DATA(fid) from fuser where fIntroUser_id="+fuserid+" \n");
	...
	public Map getTotalBalanceUser() {
		Map map = new HashMap(); 
		try {
			String queryString = "select sum(fbalanceCny) a,sum(fbalanceTMC) b from Fuser ";
	...
	public List<Object[]> getRankingMap(){
		...
		String queryString = "SELECT fintrouser_id,total from rankview where total>0 ORDER BY total desc";
		Query queryObject = getSession().createSQLQuery(queryString);
	...
	public int findMaxGlobalUserId(){
		try {
			String queryString = "SELECT max(global_user_id) from Fuser";
			Query queryObject = getSession().createSQLQuery(queryString);
			List list = queryObject.list();
			if(list!=null){
...
//		String sql = "select user from Fuser user, Ticketlog log  where ((user.fid = log.ticket.fuser.fid and log.origin != 0) or log is null ) and fisTelephoneBind = ? and fisMailValidate = ? and fhasRealValidate = ? and fregisterTime between ? and ?";
		String sql = "from Fuser user where fisTelephoneBind = ? and fisMailValidate = ? and fhasRealValidate = ? and fregisterTime between ? and ? and user.fid not in (select log.ticket.fuser.fid from Ticketlog log where origin = ?)";
