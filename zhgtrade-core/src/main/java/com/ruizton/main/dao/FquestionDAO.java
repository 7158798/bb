...
public class FquestionDAO extends HibernateDaoSupport {
	...
	public int countForPage(String name, String phoneNum, Short status, Short type, String startDate, String endDate){
		...
		sqlBuf.append("select count(*) from fquestion q where 1=1");
		if(StringUtils.hasText(name)){
			sqlBuf.append(" and q.fname =:name");
		}
		if(StringUtils.hasText(phoneNum)){
			sqlBuf.append(" and q.ftelephone =:phone");
		}
		if(null != status){
			sqlBuf.append(" and q.fstatus = ").append(status);
		}
		if(null != type){
			sqlBuf.append(" and q.ftype = ").append(type);
		}
		if(StringUtils.hasText(startDate)){
			sqlBuf.append(" and q.fcreateTime >=:start");
		}
		if(StringUtils.hasText(endDate)){
			sqlBuf.append(" and q.fcreateTime <=:end");
		}
