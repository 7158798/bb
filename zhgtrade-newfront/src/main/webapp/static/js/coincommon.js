...
function userSign(){
	var url = "/user/sign.html?random="
			+ Math.round(Math.random() * 100);
	jQuery.post(url,null,function(data){
	var callback={okBack:function(){window.location.href = "/";}};
	if (data.resultCode == 1) {
			okcoinAlert(data.resultValue, null, callback, "确定");
			isTrue = true;
		} else if (data.resultCode == 0) {
			okcoinAlert("签到成功", null, callback, "确定");
			isTrue = true;
		}
   },"json") ;
}

jQuery(".foBox li").hover(function(){
	jQuery(this).find(".ewm").stop(false,true).animate({"left":"-200px"})
},function(){
	jQuery(this).find(".ewm").stop(false,true).animate({"left":"80px"})
});
...
function showlogin(id){
	...
	jQuery("#okcoinPop").load("/user/login.html?type="+id,function (data){
		dialogBoxShadow();
		showDialog(id);
	});
}
...
function okcoinAlert(str,pro,callback,btnTitle) {
	 ...
	 */
		...
		tempStr.push('<div class="orderFloorTitle"><span style="float:left;">&nbsp;&nbsp;温馨提示</span><span style="float:right;"><a id="alertClose" href="javascript:void(0);" class="dialog_closed" title="关闭"></a></span></div>');
		...
		if(!!!pro){
			...
			tempStr.push('&nbsp;&nbsp;<input id="alertNo" class="button button-false" type="button" value="残忍拒绝" title="残忍拒绝"/></div>');
		}
...
function checkRegUserName(){
	...
	jQuery.get(url,null,function(data){
		if(data == 0){
			if(regType == 0){
				desc = "您的手机号已存在，如果这是您自己的手机号，请<a target='_blank' href='http://wpa.qq.com/msgrd?v=3&uin=*fake hank*&site=qq&menu=yes' title='联系QQ:2260505979'>联系客服</a>";
			}else{
				desc = "邮箱已存在";
			}
			document.getElementById("regNameResult").className="error orange";
...
function checkPassword(){
	...
	if(desc!=""){
		document.getElementById("regPwdResult").className="error orange";
		...
		document.getElementById("regPwdResult").className="success";
		document.getElementById("regPwdResult").innerHTML="";
		if(document.getElementById("registerErrorTips")!=null){
			document.getElementById("registerErrorTips").innerHTML="&nbsp;";
		}
	...
	function checkRePassword(){
		...
		if(desc!=""){
			document.getElementById("regRePwdResult").className="error orange";
			...
			document.getElementById("regRePwdResult").className="success";
			document.getElementById("regRePwdResult").innerHTML="";
			if(document.getElementById("registerErrorTips")!=null){
				document.getElementById("registerErrorTips").innerHTML="&nbsp;";
			}
	...
	function checkValidateCode(){
		...
		if(desc!=""){
			document.getElementById("validateCodeResult").className="error orange";
			document.getElementById("validateCodeResult").innerHTML="";
			document.getElementById("validateCodeResult").innerHTML=desc;
			if(document.getElementById("registerErrorTips")!=null){
				document.getElementById("registerErrorTips").innerHTML=desc;
			}
			return false;
		}else{
			document.getElementById("validateCodeResult").className="success";
			document.getElementById("validateCodeResult").innerHTML="";
			if(document.getElementById("registerErrorTips")!=null){
				document.getElementById("registerErrorTips").innerHTML="&nbsp;";
			}
	...
	function regSubmit(){
		if(!document.getElementById("agree").checked){
			document.getElementById("regNameResult").className="error orange";
			document.getElementById("regNameResult").innerHTML="";
			document.getElementById("regNameResult").innerHTML="请阅读并同意用户协议";
			return;
		}
		var flag = checkRegUserNameNoJquery() && checkPassword() && checkRePassword() &&checkValidateCode() ;
		if(flag==true){
			var regUserName = trim(document.getElementById("regUserName").value);
			...
			jQuery.get(urlName,null,function(data){
				if(data == 0){
					...
					var param={regName:regUserName,password:pwd,regType:regType,vcode:validateCode,intro_user:document.getElementById("introUser").value};
					jQuery.post(url,param,function(data){
						if(data < 0){
							...
							if(data==-20){
								...
								var im = document.getElementById("validateImages") ;
								if(im!=null){
									im.src = "/servlet/ValidateImageServlet?"+Math.random() ;
								}
							}else{
								if(data == -2){
									if(regType == 0){
									...
									if(document.getElementById("registerErrorTips")!=null){
										document.getElementById("registerErrorTips").innerHTML="您的浏览器还未开启COOKIE,请设置启用COOKIE功能";
									}
								}
								document.getElementById("regNameResult").className="error orange";
								document.getElementById("regNameResult").innerHTML="";
...
/** *******QQ登录****************************** */	
					...
					jQuery.each(ticker,function(key,value){
						...
						if(element!=null){
							if(parseFloat(element.innerHTML)>parseFloat(value)){
								img.attr("src",staticFileUrl+"/static/front/images/down.png") ;
							}else if(parseFloat(element.innerHTML)<parseFloat(value)){
								img.attr("src",staticFileUrl+"/static/front/images/up.png") ;
							}else{
								img.attr("src",staticFileUrl+"/static/front/images/blank.png") ;
							}
	 ...
	 *            modify by hank
	 */
	var secs = 121, secsHandler;
	function sendMsgCode(type,button_id){
		var button = document.getElementById(button_id) ;
		var url = "/captcha/sendMsg.html?type="+type+"&random="+Math.round(Math.random()*100);
		jQuery.post(url,null,function(data){
			if(data != null ){
				if(data == -3){
					tm_dialog({content : "短信验证码错误多次，请2小时后再试！", width:260, height:200, showDoubleButton : false, showSingleButton : true});
				}else if(data == -2){
					tm_dialog({content : "你还没有绑定手机！", width:260, height:200, showDoubleButton : false, showSingleButton : true});
				}else if(data == 0){
	...
	function updateNumber(num,button_id) {
		var button = document.getElementById(button_id) ;
		if (num == secs) {
			...
			if('button' === button.type){
				button.value = printnr +"s";
			}else{
				button.innerHTML = printnr +"s";
			}
		}
	}
...
function indexLoginOnblur(indexLoginName, indexLoginTips){
...
function loginIndexSubmit(indexLoginName, indexLoginPwd, indexLoginTips){
...
function startmarquee(lh,speed,delay,ID) {
	...
	function postValidateMail(){
		...
		if(data!=null){
			if(data == 0){
				...
				okcoinAlert("半小时内只能发送一次验证邮件！","",null);
			}
		}
		},"json");
	}
	...
	function ff(src)
	{
		return Math.round(src*Math.pow(10, 4))/Math.pow(10, 4);
	}
