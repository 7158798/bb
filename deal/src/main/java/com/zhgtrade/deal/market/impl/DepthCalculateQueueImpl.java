...
import com.zhgtrade.deal.Enum.EntrustStatusEnum;
...
//        executorService.execute(new SyncMarketThread());
    ...
    private void updateDepthData(int i, FentrustData message, int addOrMinus) {
        ...
        final double fleftCount = message.getFleftCount() * addOrMinus;
        final double money = message.getFprize() * fleftCount;
...
//    int count = 0;
    ...
    private void syncDepthEntrustToRedis(int fviFid, int type, int deep) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            messages[i] = new double[]{data.getFprize(), data.getFleftCount(), data.getFamount()};
        }
    ...
    class SyncMarketThread implements Runnable {
        ...
        private void generateJSON(StringBuilder sb, List<FentrustData> depth) {
            ...
            for (int i = 0; i < depth.size(); i++) {
                ...
                sb.append("[" + data.getFprize() + "," + new BigDecimal(count).setScale(4, BigDecimal.ROUND_HALF_UP).doubleValue() + "]");
            }
        }
